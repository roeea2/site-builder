...
export default async function handler(req, res) {
  ...
  // 1. Optionally verify with Cardcom (non-fatal if it fails)
  let cardcomOk = false;
  if (lowProfileCode) {
    try {
  ...
  // 3. Send notification email to Roee
    ...
    try {
      await fetch('https://api.resend.com/emails', {
        ...
        body: JSON.stringify({
          ...
          to:      'roeea2@gmail.com',
          subject: `תשלום חדש – ${lead.restaurant_name}`,
          html: `
            <div dir="rtl" style="font-family:sans-serif;max-width:500px;margin:auto">
              <h2 style="color:#2e7d32">תשלום התקבל!</h2>
              <table style="border-collapse:collapse;width:100%">
                <tr><td style="padding:8px;font-weight:bold">מסעדה</td><td style="padding:8px">${lead.restaurant_name}</td></tr>
              ...
              <p style="margin-top:24px;color:#555">הפעל <code>/deploy-restaurant</code> כדי לבנות את האתר.</p>
            </div>
          `,
        }),
      });
  ...
  res.status(200).json({ ok: true, restaurant: lead?.restaurant_name });
}
