// Public config injected by build
...
function collectOpeningHours() {
  ...
  DAYS.forEach(day => {
    schedule[day] = {
      ...
      from: document.getElementById(`hours-from-${day}`)?.value || '09:00',
      to:   document.getElementById(`hours-to-${day}`)?.value   || '21:00',
    };
  });
...
function collectServices() {
  return [...document.querySelectorAll('input[name="services"]:checked')].map(cb => cb.value);
}
...
contactForm?.addEventListener('submit', async e => {
  ...
  try {
    ...
    const barberName = document.getElementById('input-barber').value.trim();
    const email      = document.getElementById('input-email').value.trim();
    const services   = collectServices();
    const userDesc   = document.getElementById('input-description').value.trim();
    const description = services.length
      ? `שירותים: ${services.join(', ')}${userDesc ? '. ' + userDesc : ''}`
      : userDesc || null;

    submittedData = { name, phone, barberName, email };
    ...
    const res = await fetch(`${SUPABASE_URL}/rest/v1/leads`, {
      ...
      body: JSON.stringify({
        project_name:    'barbers',
        full_name:       name,
        phone:           phone,
        restaurant_name: barberName,
        email:           email || null,
        logo_url:        logoUrl,
        ...
        reservation_url: document.getElementById('input-booking-url').value.trim() || null,
    ...
    fetch('/api/notify-lead', {
      ...
      body: JSON.stringify({
        ...
        primary_color:   document.getElementById('input-primary-color').value,
        opening_hours:   collectOpeningHours(),
        booking_url:     document.getElementById('input-booking-url').value.trim() || null,
        social_facebook: document.getElementById('input-facebook').value.trim()  || null,
        social_instagram:document.getElementById('input-instagram').value.trim() || null,
        social_tiktok:   document.getElementById('input-tiktok').value.trim()    || null,
        logo_url:        logoUrl,
  ...
  try {
    const res = await fetch('/api/create-payment', {
      ...
      body: JSON.stringify({
        ...
        barberName: submittedData.barberName,
    ...
    if (data.url) {
      window.location.href = '/payment?url=' + encodeURIComponent(data.url)
        + '&barber=' + encodeURIComponent(submittedData.barberName || '');
    } else {
    ...
    btn.disabled = false;
    setTimeout(() => {
      btn.innerHTML = '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="vertical-align:middle;margin-left:6px"><rect x="1" y="4" width="22" height="16" rx="2"/><line x1="1" y1="10" x2="23" y2="10"/></svg> תשלום מאובטח ←';
    }, 3000);
  }
});
